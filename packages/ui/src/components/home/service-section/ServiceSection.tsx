    ...
    <section className="home-section min-h-[50vh] pb-4" id="services">
      ...
      <section className="mt-24 flex max-md:flex-col">
        <aside className="flex-1 flex flex-col items-end max-md:items-center max-md:mb-6 ">
          ...
          ></div>
          <ServiceCard extraClass="-translate-y-16 md:block hidden" imgSrc={order} cardNumber={2} title={card[1]?.title || "abc"} description={card[1]?.description || "xyz"} />
        </aside>
        <aside className="flex-1 flex flex-col md:pl-6 max-md:items-center">
          <ServiceCard imgSrc={scan} cardNumber={1} title={card[0]?.title || "abc"} description={card[0]?.description || "xyz"} />
          <ServiceCard extraClass="md:hidden mt-6" imgSrc={order} cardNumber={2} title={card[1]?.title || "abc"} description={card[1]?.description || "xyz"} />
          <ServiceCard extraClass="mt-6" imgSrc={bills} cardNumber={3} title={card[2]?.title || "abc"} description={card[2]?.description || "xyz"} />
        </aside>
      </section>
    </section>
  );
